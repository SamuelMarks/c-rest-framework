...
c_rest_error_t c_rest_websocket_upgrade(struct c_rest_request *req,
  ...
  rc = c_rest_request_get_header(req, "Upgrade", &upgrade);
  if (rc != C_REST_OK)
    return rc;
  ...
  rc = c_rest_request_get_header(req, "Sec-WebSocket-Key", &ws_key);
  if (rc != C_REST_OK)
    return rc;
...
c_rest_error_t c_rest_websocket_generate_accept(const char *ws_key,
  ...
  if (concat_len >= sizeof(concat_buf)) {
    return C_REST_ERROR_GENERIC; /* Key too long */
  }
...
c_rest_error_t c_rest_websocket_parse_frame_header(
  ...
  if (out_header->payload_length == 126) {
    if (data_len < 4) {
      return C_REST_ERROR_GENERIC; /* Need 2 more bytes */
    }
    ...
    if (data_len < 10) {
      return C_REST_ERROR_GENERIC; /* Need 8 more bytes */
    }
