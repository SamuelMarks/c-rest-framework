...
int c_rest_websocket_upgrade(struct c_rest_request *req, /* GCOVR_EXCL_LINE */
...
int c_rest_websocket_generate_accept(const char *ws_key,
  ...
  if (concat_len >= sizeof(concat_buf)) {        /* GCOVR_EXCL_LINE */
    return 1; /* Key too long */                 /* GCOVR_EXCL_LINE */
  }
...
int c_rest_websocket_parse_frame_header(/* GCOVR_EXCL_LINE */
  ...
  if (!data || data_len < 2 || !out_header) { /* GCOVR_EXCL_LINE */
    return 1; /* Need at least 2 bytes */     /* GCOVR_EXCL_LINE */
  }
  ...
  if (out_header->payload_length == 126) { /* GCOVR_EXCL_LINE */
    if (data_len < 4) {                    /* GCOVR_EXCL_LINE */
      return 1; /* Need 2 more bytes */    /* GCOVR_EXCL_LINE */
    }
    ext_len_16 =
    ...
    if (data_len < 10) {                            /* GCOVR_EXCL_LINE */
      return 1; /* Need 8 more bytes */             /* GCOVR_EXCL_LINE */
    }
  ...
  if (out_header->masked) {                        /* GCOVR_EXCL_LINE */
    if (data_len < offset + 4) {                   /* GCOVR_EXCL_LINE */
      return 1; /* Need 4 bytes for masking key */ /* GCOVR_EXCL_LINE */
    }
...
int c_rest_websocket_unmask_payload(unsigned char *payload,
...
int c_rest_websocket_serialize_frame_header(/* GCOVR_EXCL_LINE */
                                            const struct
                                            c_rest_websocket_frame_header
                                                *header,
